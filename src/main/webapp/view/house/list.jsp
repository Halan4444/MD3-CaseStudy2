...
<div class="container-fluid mx-auto">
    <div class="row justify-content-between py-1">
        <div class="col-4">
            <a class="btn btn-primary" href="/houses?action=create" role="button"> + Create new service</a>
            <a class="btn btn-primary" href="/view/house/detail.jsp" role="button"> + Homestay Detail</a>
        </div>
    </div>
    <table id="myTable2" class="table table-striped table-bordered table-dark table-hover" style="width: 100%">
        ...
        <tr>
            ...
            <th scope="col">Basic Utility</th>
            <th scope="col">House standard</th>
            <th scope="col">Status</th>
            <th scope="col">Price</th>
            <th scope="col">Extra Utilitis</th>
        ...
        <c:forEach var="i" begin="0" end="${houseList.size()-1}">
            <tr>
                <td>${(i+1)}</td>
                <td>
                    <a href="/houses?action=view&id=${houseList.get(i).id}">
                            ${houseList.get(i).name}
                    </a>
                </td>
                <td> ${typeList.get(i).name} </td>
                <td> ${houseList.get(i).designStyle} </td>
                <td> ${houseList.get(i).address} </td>
                <td> ${houseList.get(i).maximumPeople} </td>
                <td> ${houseList.get(i).basicUtility} </td>
                <td> ${standardList.get(i).name} </td>
                <td> ${houseList.get(i).status} </td>
                <td>
                    <fmt:setLocale value="vi_VN"/>
                    <fmt:formatNumber value="${houseList.get(i).price}" type="currency"/>
                </td>
                <td> ${houseList.get(i).extraUtility} </td>
                <td>
                    <a class="btn btn-warning" role="button" href="/houses?action=edit&id=${houseList.get(i).id}">
                        edit
                    </a>
                </td>
                <td>
